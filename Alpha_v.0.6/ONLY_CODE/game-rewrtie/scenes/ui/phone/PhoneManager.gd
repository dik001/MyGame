...
func _ensure_open_phone_action() -> void:
	if not InputMap.has_action(OPEN_PHONE_ACTION):
		InputMap.add_action(OPEN_PHONE_ACTION)

	if _action_has_key(OPEN_PHONE_ACTION, KEY_UP):
		return

	var key_event := InputEventKey.new()
	key_event.physical_keycode = KEY_UP
	key_event.keycode = KEY_UP
	InputMap.action_add_event(OPEN_PHONE_ACTION, key_event)
...
func _action_has_key(action_name: StringName, keycode: int) -> bool:
	for event in InputMap.action_get_events(action_name):
		...
		if int(key_event.physical_keycode) == keycode or int(key_event.keycode) == keycode:
			return true
