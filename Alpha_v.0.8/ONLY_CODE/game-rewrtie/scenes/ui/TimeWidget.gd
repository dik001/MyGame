...
func set_total_minutes(total_minutes: int) -> void:
	...
	var hours := int(floor(float(normalized_total) / float(MINUTES_PER_HOUR)))
...
func _refresh_display() -> void:
	_set_digit(hour_tens, int(floor(float(_hours) / 10.0)))
	_set_digit(hour_ones, _hours % 10)
	_set_digit(minute_tens, int(floor(float(_minutes) / 10.0)))
