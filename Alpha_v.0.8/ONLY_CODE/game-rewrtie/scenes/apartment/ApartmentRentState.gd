...
const FIRST_CYCLE_RENT_AMOUNT: int = 1700
...
func get_rent_amount() -> int:
	return _resolve_cycle_rent_amount(_current_cycle)
...
func pay_current_rent() -> Dictionary:
	...
	var current_rent_amount: int = _resolve_cycle_rent_amount(_current_cycle)
	...
	if not PlayerEconomy.can_afford_cash(current_rent_amount):
		var failure_result: Dictionary = _make_payment_result(false, "insufficient_funds", "Недостаточно денег для оплаты аренды.")
		failure_result["required_amount"] = current_rent_amount
		failure_result["current_dollars"] = PlayerEconomy.get_cash_dollars()
		failure_result["current_cash_dollars"] = PlayerEconomy.get_cash_dollars()
		return failure_result

	if not PlayerEconomy.spend_cash_dollars(current_rent_amount):
	...
	var result: Dictionary = {
		...
		"current_dollars": PlayerEconomy.get_cash_dollars(),
		"current_cash_dollars": PlayerEconomy.get_cash_dollars(),
	}
...
func _initialize_from_current_time() -> void:
	...
	_current_cycle = _build_cycle(
		1,
		GameTime.get_day() + FIRST_CYCLE_OFFSET_DAYS,
		GameTime.get_absolute_minutes(),
		FIRST_CYCLE_RENT_AMOUNT
	)


func _build_cycle(cycle_index: int, due_day: int, created_absolute_minutes: int, rent_amount: int = RENT_AMOUNT) -> Dictionary:
	return {
		"cycle_index": max(1, cycle_index),
		"due_day": max(1, due_day),
		"rent_amount": max(0, rent_amount),
		"is_paid": false,
		"is_overdue": false,
...
func _process_current_time(_current_absolute_minutes: int, current_day: int, emit_signals: bool = true) -> Dictionary:
	...
	var rent_amount: int = _resolve_cycle_rent_amount(_current_cycle)
	var rent_amount_text: String = _format_rent_amount(rent_amount)
	...
	if current_day >= reminder_day and not bool(_current_cycle.get("reminder_generated", false)):
		changes["landlord_changed"] = _append_landlord_message(
			"Напоминаю: завтра нужно оплатить аренду квартиры. Сумма: %s." % rent_amount_text,
		...
		changes["calendar_changed"] = _append_calendar_event(
			...
			"Завтра нужно оплатить аренду квартиры. Сумма: %s." % rent_amount_text,
			reminder_day,
			_build_absolute_minutes_for_day(reminder_day, REMINDER_HOUR),
			"rent",
			cycle_index,
		...
		if current_day == reminder_day:
			changes["emit_tomorrow"] = true

	if current_day >= due_day and not bool(_current_cycle.get("due_generated", false)):
		changes["landlord_changed"] = _append_landlord_message(
			"Сегодня срок оплаты аренды. Сумма: %s." % rent_amount_text,
	...
	if should_be_overdue and not bool(_current_cycle.get("overdue_generated", false)):
		...
		changes["landlord_changed"] = _append_landlord_message(
			"Аренда просрочена. Пожалуйста, оплатите %s как можно скорее." % rent_amount_text,
	...
	if should_be_overdue:
		...
		while generated_followups < expected_followups:
			...
			changes["landlord_changed"] = _append_landlord_message(
				"Аренда всё ещё просрочена. Пожалуйста, оплатите %s как можно скорее." % rent_amount_text,
			...
			changes["calendar_changed"] = _append_calendar_event(
				"Просрочка аренды",
				"Просрочка по аренде сохраняется. Оплатите %s при первой возможности." % rent_amount_text,
				followup_day,
				_build_absolute_minutes_for_day(followup_day, OVERDUE_FOLLOWUP_HOUR),
				"rent",
				cycle_index,
...
func _emit_change_set(changes: Dictionary) -> void:
	...
	if bool(changes.get("emit_tomorrow", false)):
		rent_due_tomorrow.emit()
...
func _merge_change_sets(left: Dictionary, right: Dictionary) -> Dictionary:
	return {
		...
		"emit_tomorrow": bool(left.get("emit_tomorrow", false)) or bool(right.get("emit_tomorrow", false)),
...
func _resolve_cycle_rent_amount(cycle_data: Dictionary) -> int:
	if cycle_data.is_empty():
		return RENT_AMOUNT

	return int(cycle_data.get("rent_amount", RENT_AMOUNT))


func _format_rent_amount(amount: int) -> String:
...
func build_save_data() -> Dictionary:
	return {
		...
		"current_cycle": _current_cycle.duplicate(true),
		"landlord_messages": _landlord_messages.duplicate(true),
		"calendar_events": _calendar_events.duplicate(true),
		"rent_history": _rent_history.duplicate(true),
		"next_message_id": _next_message_id,
		"next_calendar_event_id": _next_calendar_event_id,
	}


func apply_save_data(data: Dictionary) -> void:
	_is_initialized = bool(data.get("is_initialized", false))
	_current_cycle = SaveDataUtils.sanitize_dictionary(data.get("current_cycle", {}))
	_landlord_messages = []
	_calendar_events = []
	_rent_history = []

	for entry_variant in SaveDataUtils.sanitize_array(data.get("landlord_messages", [])):
		if entry_variant is Dictionary:
			_landlord_messages.append((entry_variant as Dictionary).duplicate(true))

	for entry_variant in SaveDataUtils.sanitize_array(data.get("calendar_events", [])):
		if entry_variant is Dictionary:
			_calendar_events.append((entry_variant as Dictionary).duplicate(true))

	for entry_variant in SaveDataUtils.sanitize_array(data.get("rent_history", [])):
		if entry_variant is Dictionary:
			_rent_history.append((entry_variant as Dictionary).duplicate(true))

	_next_message_id = max(1, int(data.get("next_message_id", 1)))
	_next_calendar_event_id = max(1, int(data.get("next_calendar_event_id", 1)))
	_refresh_unread_landlord_count()
	rent_state_changed.emit()
	landlord_feed_changed.emit()
	calendar_events_changed.emit()
...
func reset_state() -> void:
	...
	_landlord_messages.clear()
	_calendar_events.clear()
	_rent_history.clear()
	_unread_landlord_count = 0
	_next_message_id = 1
	_next_calendar_event_id = 1
	rent_state_changed.emit()
	landlord_feed_changed.emit()
