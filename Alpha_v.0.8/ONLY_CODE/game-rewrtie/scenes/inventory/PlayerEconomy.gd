...
var _default_cash_dollars: int = DEFAULT_STARTING_CASH_DOLLARS
var _default_bank_dollars: int = DEFAULT_STARTING_BANK_DOLLARS
...
func _ready() -> void:
	_default_cash_dollars = cash_dollars
	_default_bank_dollars = bank_dollars
...
func can_afford_bank(amount: int) -> bool:
	if amount <= 0:
		return true
...
func spend_bank_dollars(amount: int, count_as_expense: bool = true, bookkeeping_day: int = -1) -> bool:
	if amount <= 0:
		return true

	if not can_afford_bank(amount):
		return false
...
func remove_bank_dollars(amount: int, count_as_expense: bool = true, bookkeeping_day: int = -1) -> bool:
	return spend_bank_dollars(amount, count_as_expense, bookkeeping_day)
...
func withdraw_bank_dollars(amount: int) -> bool:
	...
	if not spend_bank_dollars(amount, false):
		return false
...
func deposit_cash_to_bank(amount: int) -> bool:
	if amount <= 0:
		return false
...
func build_save_data() -> Dictionary:
	return {
		...
		"bank_dollars": bank_dollars,
		"daily_totals_by_day": _daily_totals_by_day.duplicate(true),
		"current_observed_day": _current_observed_day,
	}


func apply_save_data(data: Dictionary) -> void:
	cash_dollars = max(0, int(data.get("cash_dollars", _default_cash_dollars)))
	bank_dollars = max(0, int(data.get("bank_dollars", _default_bank_dollars)))
	_daily_totals_by_day = SaveDataUtils.sanitize_dictionary(data.get("daily_totals_by_day", {}))
	_current_observed_day = max(1, int(data.get("current_observed_day", GameTime.get_day())))
	_emit_cash_changed()
	_emit_bank_changed()
...
func reset_state() -> void:
	cash_dollars = max(0, _default_cash_dollars)
	bank_dollars = max(0, _default_bank_dollars)
	_daily_totals_by_day.clear()
	_current_observed_day = max(1, GameTime.get_day())
	_emit_cash_changed()
	_emit_bank_changed()
	_emit_daily_summary_changed()
