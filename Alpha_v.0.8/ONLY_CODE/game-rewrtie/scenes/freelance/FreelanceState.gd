...
func append_bank_history_entry(
	source: String,
	title: String,
	amount: int,
	status: String,
	notification_text: String = "",
	absolute_minutes: int = -1,
	day_id: int = -1
) -> Dictionary:
	var resolved_title: String = title.strip_edges()
	var resolved_source: String = source.strip_edges().to_lower()
	var resolved_status: String = status.strip_edges().to_lower()
	var resolved_absolute_minutes: int = absolute_minutes
	var resolved_day_id: int = day_id

	if resolved_title.is_empty():
		resolved_title = "Без названия"
	...
	if resolved_absolute_minutes < 0:
		resolved_absolute_minutes = GameTime.get_absolute_minutes()

	if resolved_day_id <= 0:
		resolved_day_id = GameTime.get_day()

	var resolved_notification: String = notification_text.strip_edges()

	if resolved_notification.is_empty():
		var amount_prefix: String = "-" if amount < 0 else ""
		resolved_notification = "%s: %s$%d" % [resolved_title, amount_prefix, abs(amount)]

	var transfer_entry: Dictionary = {
		"source": resolved_source,
		"order_title": resolved_title,
		"amount": amount,
		"absolute_minutes": resolved_absolute_minutes,
		"day_id": resolved_day_id,
		"status": resolved_status,
		"notification_text": resolved_notification,
	}
	...
	_last_bank_notification = resolved_notification
	bank_history_changed.emit()
...
func _resolve_order_result(order_id: int, accuracy: float, forced_fail: bool) -> Dictionary:
	...
	energy_delta = _resolve_work_energy_delta(energy_delta)
	_apply_energy_delta(energy_delta, result_status)

	if payout > 0:
		PlayerEconomy.add_bank_dollars(payout, true, completion_day_id)
		append_bank_history_entry(
			"freelance",
			String(order.get("title", "")),
			payout,
			"credited",
			"Перевод за заказ \"%s\": $%d" % [String(order.get("title", "")), payout],
...
func build_save_data() -> Dictionary:
	return {
		"current_generated_day_id": _current_generated_day_id,
		"current_day_orders": _duplicate_dictionary_array(_current_day_orders),
		"completed_today_count": _completed_today_count,
		"completed_today_day_id": _completed_today_day_id,
		"service_rating": _service_rating,
		"moderation_level": _moderation_level,
		"moderation_xp": _moderation_xp,
		"recent_history": _duplicate_dictionary_array(_recent_history),
		"bank_history": _duplicate_dictionary_array(_bank_history),
		"last_bank_notification": _last_bank_notification,
		"active_conditions": _active_conditions.duplicate(true),
		"last_observed_day_id": _last_observed_day_id,
	}


func apply_save_data(data: Dictionary) -> void:
	_current_generated_day_id = max(0, int(data.get("current_generated_day_id", 0)))
	_current_day_orders = []

	for order_variant in SaveDataUtils.sanitize_array(data.get("current_day_orders", [])):
		if order_variant is Dictionary:
			_current_day_orders.append((order_variant as Dictionary).duplicate(true))

	_completed_today_count = max(0, int(data.get("completed_today_count", 0)))
	_completed_today_day_id = max(0, int(data.get("completed_today_day_id", 0)))
	_service_rating = clampi(int(data.get("service_rating", DEFAULT_SERVICE_RATING)), 0, MAX_SERVICE_RATING)
	_moderation_level = clampi(int(data.get("moderation_level", DEFAULT_MODERATION_LEVEL)), DEFAULT_MODERATION_LEVEL, MAX_MODERATION_LEVEL)
	_moderation_xp = max(0, int(data.get("moderation_xp", DEFAULT_MODERATION_XP)))
	_recent_history = []
	_bank_history = []

	for entry_variant in SaveDataUtils.sanitize_array(data.get("recent_history", [])):
		if entry_variant is Dictionary:
			_recent_history.append((entry_variant as Dictionary).duplicate(true))

	for entry_variant in SaveDataUtils.sanitize_array(data.get("bank_history", [])):
		if entry_variant is Dictionary:
			_bank_history.append((entry_variant as Dictionary).duplicate(true))

	_last_bank_notification = String(data.get("last_bank_notification", "")).strip_edges()
	_active_conditions = SaveDataUtils.sanitize_dictionary(data.get("active_conditions", {}))
	_last_observed_day_id = max(0, int(data.get("last_observed_day_id", 0)))
	orders_changed.emit()
	service_rating_changed.emit(_service_rating)
	skill_changed.emit(_moderation_level, _moderation_xp)
	history_changed.emit()
...
func reset_state() -> void:
	...
	_current_day_orders.clear()
	_completed_today_count = 0
	_completed_today_day_id = 0
	_service_rating = DEFAULT_SERVICE_RATING
	_moderation_level = DEFAULT_MODERATION_LEVEL
	_moderation_xp = DEFAULT_MODERATION_XP
	_recent_history.clear()
	_bank_history.clear()
	_last_bank_notification = ""
	_active_conditions.clear()
	_last_observed_day_id = 0
	orders_changed.emit()
	service_rating_changed.emit(_service_rating)
	skill_changed.emit(_moderation_level, _moderation_xp)
	history_changed.emit()
