...
var _dev_console: LeChatDevConsole = null


func _ready() -> void:
	visible = false
	_dev_console = LeChatDevConsole.new(self)
	...
	if not message_line_edit.gui_input.is_connected(_on_message_line_edit_gui_input):
		message_line_edit.gui_input.connect(_on_message_line_edit_gui_input)
...
func _ensure_selected_chat() -> void:
	if _is_dev_console_active():
		_selected_chat_id = _get_dev_chat_id()
		return

	if _selected_chat_id.is_empty():
		_selected_chat_id = LANDLORD_CHAT_ID

	if _selected_chat_id == _get_dev_chat_id():
		_selected_chat_id = LANDLORD_CHAT_ID


func _build_chat_entries() -> Array[Dictionary]:
	var chats: Array[Dictionary] = []
	var chat_id: String = _get_dev_chat_id() if _is_dev_console_active() else LANDLORD_CHAT_ID
	var messages: Array[Dictionary] = _get_combined_chat_messages(chat_id)
	var last_message: Dictionary = {}

	if not messages.is_empty():
		last_message = messages[messages.size() - 1]

	var unread_count: int = 0 if _is_dev_console_active() else _get_unread_landlord_count()
	var preview_text: String = _build_preview_text(last_message)

	if preview_text.is_empty() and _is_dev_console_active():
		preview_text = "Введите /help"
	...
	chats.append({
		"chat_id": chat_id,
		"display_name": _dev_console.get_display_name() if _is_dev_console_active() else LANDLORD_DISPLAY_NAME,
		"preview": preview_text,
		"unread_count": unread_count,
		"selected": _selected_chat_id == chat_id,
		"status": _dev_console.get_status_text() if _is_dev_console_active() else _build_header_status_text(),
	})
...
func _refresh_header(chats: Array[Dictionary]) -> void:
	...
	if not _is_dev_console_active() and _selected_chat_id == LANDLORD_CHAT_ID:
...
func _rebuild_messages() -> void:
	...
	if _selected_chat_id != LANDLORD_CHAT_ID and _selected_chat_id != _get_dev_chat_id():
		_add_state_label("Этот чат пока не поддерживается")
		return

	if _selected_chat_id == LANDLORD_CHAT_ID and _rent_state == null:
...
func _refresh_input_state() -> void:
	var can_type: bool = (
		_selected_chat_id == _get_dev_chat_id()
		if _is_dev_console_active()
		else (_rent_state != null and _selected_chat_id == LANDLORD_CHAT_ID)
	)

	message_line_edit.editable = can_type
	message_line_edit.placeholder_text = (
		_dev_console.get_input_placeholder()
		if _is_dev_console_active()
		else "Напишите сообщение..."
	) if can_type else "Чат недоступен"
...
func _send_current_message() -> void:
	...
	if _dev_console != null and _dev_console.should_handle_submission(text):
		_handle_dev_console_submission(text)
	...
	_append_local_message(
		_selected_chat_id,
		_build_local_message(text, PLAYER_SENDER_ID, PLAYER_DISPLAY_NAME, "player_reply", true, _selected_chat_id)
	)
	message_line_edit.clear()
	refresh_view()
...
func _build_local_message(
	...
	return {
		...
		"chat_id": chat_id_override if not chat_id_override.is_empty() else _selected_chat_id,
		"sender_id": sender_id,
...
func _append_local_notice(text: String) -> void:
	_append_local_message(
		LANDLORD_CHAT_ID,
		_build_local_message(
			text,
			LOCAL_NOTICE_SENDER_ID,
			LOCAL_NOTICE_DISPLAY_NAME,
			"local_notice",
			false,
			LANDLORD_CHAT_ID
		)
...
func _mark_current_chat_as_read_if_needed() -> void:
	...
	if _is_dev_console_active() or _selected_chat_id != LANDLORD_CHAT_ID:
		return
...
func _on_message_line_edit_gui_input(event: InputEvent) -> void:
	if not _is_dev_console_active():
		return
	...
	if key_event.keycode == KEY_UP:
		message_line_edit.text = _dev_console.navigate_history(-1, message_line_edit.text)
		message_line_edit.caret_column = message_line_edit.text.length()
		get_viewport().set_input_as_handled()
...
func _handle_dev_console_submission(text: String) -> void:
	if _dev_console == null:
		return

	var result: Dictionary = _dev_console.handle_submission(text)

	if not bool(result.get("handled", false)):
		return

	var dev_chat_id: String = _get_dev_chat_id()

	if bool(result.get("clear_messages", false)):
		_session_messages_by_chat[dev_chat_id] = []

	if bool(result.get("append_player_message", false)):
		_append_local_message(
			dev_chat_id,
			_build_local_message(
				String(result.get("player_text", "")),
				PLAYER_SENDER_ID,
				PLAYER_DISPLAY_NAME,
				"player_reply",
				true,
	...
	for message_variant in result.get("messages", []):
		_append_dev_console_message(String(message_variant))

	if bool(result.get("deactivate_console", false)):
		_dev_console.deactivate()
		_selected_chat_id = LANDLORD_CHAT_ID
		return

	_selected_chat_id = dev_chat_id


func _append_dev_console_message(text: String) -> void:
	if _dev_console == null:
		return
	...
	_append_local_message(
		...
		_build_local_message(
			text,
			_dev_console.get_sender_id(),
			_dev_console.get_sender_name(),
			"dev_console",
			false,
			dev_chat_id
		)
...
func _is_dev_console_active() -> bool:
	return _dev_console != null and _dev_console.is_active()


func _get_dev_chat_id() -> String:
	return _dev_console.get_session_chat_id() if _dev_console != null else ""
