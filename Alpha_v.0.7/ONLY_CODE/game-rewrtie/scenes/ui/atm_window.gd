...
@onready var deposit_section_label: Label = $Dim/Panel/MarginContainer/Content/DepositSectionLabel
@onready var deposit_quick_10_button: Button = $Dim/Panel/MarginContainer/Content/DepositQuickButtonsRow/Quick10Button
@onready var deposit_quick_20_button: Button = $Dim/Panel/MarginContainer/Content/DepositQuickButtonsRow/Quick20Button
@onready var deposit_quick_50_button: Button = $Dim/Panel/MarginContainer/Content/DepositQuickButtonsRow/Quick50Button
@onready var deposit_quick_100_button: Button = $Dim/Panel/MarginContainer/Content/DepositQuickButtonsRow/Quick100Button
@onready var deposit_quick_200_button: Button = $Dim/Panel/MarginContainer/Content/DepositQuickButtonsRow/Quick200Button
@onready var deposit_amount_label: Label = $Dim/Panel/MarginContainer/Content/DepositCustomRow/AmountLabel
@onready var deposit_amount_input: LineEdit = $Dim/Panel/MarginContainer/Content/DepositCustomRow/AmountInput
@onready var deposit_button: Button = $Dim/Panel/MarginContainer/Content/DepositCustomRow/DepositButton
...
func _ready() -> void:
	...
	deposit_section_label.text = "Пополнить счет"
	deposit_amount_label.text = "Сумма"
	deposit_button.text = "Пополнить"
	close_button.text = "Закрыть"
	amount_input.placeholder_text = "Введите сумму"
	deposit_amount_input.placeholder_text = "Введите сумму"
	...
	deposit_quick_10_button.text = "$10"
	deposit_quick_20_button.text = "$20"
	deposit_quick_50_button.text = "$50"
	deposit_quick_100_button.text = "$100"
	deposit_quick_200_button.text = "$200"
	...
	if not deposit_button.pressed.is_connected(_on_deposit_button_pressed):
		deposit_button.pressed.connect(_on_deposit_button_pressed)

	if not deposit_amount_input.text_submitted.is_connected(_on_deposit_amount_input_text_submitted):
		deposit_amount_input.text_submitted.connect(_on_deposit_amount_input_text_submitted)
	...
	if not deposit_quick_10_button.pressed.is_connected(_on_quick_deposit_pressed.bind(10)):
		deposit_quick_10_button.pressed.connect(_on_quick_deposit_pressed.bind(10))

	if not deposit_quick_20_button.pressed.is_connected(_on_quick_deposit_pressed.bind(20)):
		deposit_quick_20_button.pressed.connect(_on_quick_deposit_pressed.bind(20))

	if not deposit_quick_50_button.pressed.is_connected(_on_quick_deposit_pressed.bind(50)):
		deposit_quick_50_button.pressed.connect(_on_quick_deposit_pressed.bind(50))

	if not deposit_quick_100_button.pressed.is_connected(_on_quick_deposit_pressed.bind(100)):
		deposit_quick_100_button.pressed.connect(_on_quick_deposit_pressed.bind(100))

	if not deposit_quick_200_button.pressed.is_connected(_on_quick_deposit_pressed.bind(200)):
		deposit_quick_200_button.pressed.connect(_on_quick_deposit_pressed.bind(200))
...
func refresh_view() -> void:
	...
	if player_economy == null:
		cash_label.text = "Наличные: --"
		bank_label.text = "На счете: --"
		return

	cash_label.text = "Наличные: %s" % _format_money(player_economy.get_cash_dollars())
	bank_label.text = "На счете: %s" % _format_money(player_economy.get_bank_dollars())
...
func _withdraw_amount(amount: int) -> void:
	...
	if player_economy == null:
		_show_status("Банковый счет недоступен.", STATUS_COLOR_ERROR)
		return

	if not player_economy.can_withdraw_from_bank(amount):
		_show_status("На счете недостаточно денег для снятия.", STATUS_COLOR_ERROR)
		return

	if not player_economy.withdraw_bank_dollars(amount):
...
func _deposit_amount(amount: int) -> void:
	if amount <= 0:
		_show_status("Сумма должна быть больше нуля.", STATUS_COLOR_ERROR)
		return
	...
	if not player_economy.can_afford_cash(amount):
		_show_status("Недостаточно наличных для пополнения.", STATUS_COLOR_ERROR)
		return

	if not player_economy.deposit_cash_to_bank(amount):
		_show_status("Не удалось пополнить счет. Попробуйте снова.", STATUS_COLOR_ERROR)
		return

	deposit_amount_input.clear()
	_append_deposit_history(amount)
	_show_status("На счет зачислено: %s" % _format_money(amount), STATUS_COLOR_SUCCESS)


func _parse_amount(amount_field: LineEdit, empty_message: String) -> int:
	var amount_text: String = amount_field.text.strip_edges()

	if amount_text.is_empty():
		_show_status(empty_message, STATUS_COLOR_ERROR)
		return -1
...
func _append_deposit_history(amount: int) -> void:
	...
	freelance_state.call(
		...
		amount,
		"credited",
		"Пополнение через банкомат: +$%d" % amount
	)
...
func _on_withdraw_button_pressed() -> void:
	var amount: int = _parse_amount(amount_input, "Введите сумму для снятия.")
...
func _on_deposit_button_pressed() -> void:
	var amount: int = _parse_amount(deposit_amount_input, "Введите сумму для пополнения.")
...
func _on_deposit_amount_input_text_submitted(_new_text: String) -> void:
	_on_deposit_button_pressed()


func _on_quick_deposit_pressed(amount: int) -> void:
	_deposit_amount(amount)
