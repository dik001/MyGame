...
func build_save_data() -> Dictionary:
	return {
		"sms_unlocked": _sms_unlocked,
		"selected_sms_contact": _selected_sms_contact,
		"sms_contact_order": _sms_contact_order.duplicate(),
		"sms_threads": _serialize_sms_threads(),
		"contact_avatars": _serialize_contact_avatars(),
	}


func apply_save_data(data: Dictionary) -> void:
	reset_state()
	_sms_unlocked = bool(data.get("sms_unlocked", false))
	_sms_contact_order.clear()

	for contact_variant in SaveDataUtils.sanitize_array(data.get("sms_contact_order", [])):
		var contact_name := String(contact_variant).strip_edges()

		if contact_name.is_empty() or _sms_contact_order.has(contact_name):
			continue

		_sms_contact_order.append(contact_name)

	_sms_threads = _deserialize_sms_threads(data.get("sms_threads", {}))
	_contact_avatars = _deserialize_contact_avatars(data.get("contact_avatars", {}))
	_selected_sms_contact = String(data.get("selected_sms_contact", "")).strip_edges()

	if _selected_sms_contact.is_empty() and not _sms_contact_order.is_empty():
		_selected_sms_contact = _sms_contact_order[0]

	if not _sms_threads.is_empty():
		_sms_unlocked = true
...
func reset_state() -> void:
	...
	_selected_page = PhonePage.HOME
	_page_before_call = PhonePage.HOME
	_current_contact_name = DEFAULT_CONTACT_NAME
	_current_conversation_id = ""
	_current_dialogue_text = ""
	_current_avatar = null
	_call_duration_seconds = 0
	_requested_open = false
	_auto_close_after_end = false
	_sms_unlocked = false
	_selected_sms_contact = ""
	_sms_contact_order.clear()
	_sms_threads.clear()
	_contact_avatars.clear()
...
func _serialize_sms_threads() -> Dictionary:
	var result: Dictionary = {}

	for contact_name in _sms_threads.keys():
		var thread_entries: Array = []
		var thread: Array = _sms_threads.get(contact_name, [])

		for message_variant in thread:
			if not (message_variant is Dictionary):
				continue

			var message_entry: Dictionary = message_variant
			thread_entries.append({
				"text": String(message_entry.get("text", "")).strip_edges(),
				"is_player": bool(message_entry.get("is_player", false)),
			})

		result[String(contact_name)] = thread_entries

	return result


func _deserialize_sms_threads(raw_threads: Variant) -> Dictionary:
	var result: Dictionary = {}
	var threads: Dictionary = SaveDataUtils.sanitize_dictionary(raw_threads)

	for contact_key in threads.keys():
		var contact_name := String(contact_key).strip_edges()

		if contact_name.is_empty():
			continue

		var normalized_thread: Array = []

		for message_variant in SaveDataUtils.sanitize_array(threads.get(contact_key, [])):
			if not (message_variant is Dictionary):
				continue

			var message_entry: Dictionary = message_variant
			var text := String(message_entry.get("text", "")).strip_edges()

			if text.is_empty():
				continue

			normalized_thread.append({
				"text": text,
				"is_player": bool(message_entry.get("is_player", false)),
			})

		result[contact_name] = normalized_thread

		if not _sms_contact_order.has(contact_name):
			_sms_contact_order.append(contact_name)

	return result


func _serialize_contact_avatars() -> Dictionary:
	var result: Dictionary = {}

	for contact_key in _contact_avatars.keys():
		var contact_name := String(contact_key).strip_edges()
		var avatar := _contact_avatars.get(contact_key, null) as Texture2D
		var avatar_path: String = SaveDataUtils.texture_to_path(avatar)

		if contact_name.is_empty() or avatar_path.is_empty():
			continue

		result[contact_name] = avatar_path

	return result


func _deserialize_contact_avatars(raw_avatars: Variant) -> Dictionary:
	var result: Dictionary = {}
	var avatars: Dictionary = SaveDataUtils.sanitize_dictionary(raw_avatars)

	for contact_key in avatars.keys():
		var contact_name := String(contact_key).strip_edges()
		var avatar_path := String(avatars.get(contact_key, "")).strip_edges()

		if contact_name.is_empty() or avatar_path.is_empty():
			continue

		var avatar: Texture2D = SaveDataUtils.load_texture_from_path(avatar_path)

		if avatar == null:
			continue

		result[contact_name] = avatar
