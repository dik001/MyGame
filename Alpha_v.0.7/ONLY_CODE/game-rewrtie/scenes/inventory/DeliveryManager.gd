...
func build_save_data() -> Dictionary:
	return {
		"active_deliveries": _active_deliveries.duplicate(true),
		"delivered_deliveries": _delivered_deliveries.duplicate(true),
		"next_delivery_id": _next_delivery_id,
	}


func apply_save_data(data: Dictionary) -> void:
	_active_deliveries = SaveDataUtils.sanitize_array(data.get("active_deliveries", []))
	_delivered_deliveries = SaveDataUtils.sanitize_array(data.get("delivered_deliveries", []))
	_next_delivery_id = max(1, int(data.get("next_delivery_id", 1)))
	deliveries_updated.emit()


func reset_state() -> void:
	_active_deliveries.clear()
	_delivered_deliveries.clear()
	_next_delivery_id = 1
	_is_updating_deliveries = false
	deliveries_updated.emit()
