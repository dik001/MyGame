...
@onready var balance_label: Label = $MarginContainer/Content/HeaderRow/BalancesColumn/BalanceLabel
@onready var cash_label: Label = $MarginContainer/Content/HeaderRow/BalancesColumn/CashLabel
...
func _connect_state_signals() -> void:
	...
	if _player_economy != null:
		_connect_optional_signal(_player_economy, &"bank_dollars_changed", Callable(self, "_on_bank_dollars_changed"))
		_connect_optional_signal(_player_economy, &"cash_dollars_changed", Callable(self, "_on_cash_dollars_changed"))
...
func _update_balances() -> void:
	if _player_economy == null:
		balance_label.text = "На счете: --"
		cash_label.text = "Наличные: --"
		return

	var has_bank_balance: bool = false
	var has_cash_balance: bool = false
	var bank_balance: int = 0
	var cash_balance: int = 0

	if _player_economy.has_method("get_bank_dollars"):
		bank_balance = int(_player_economy.call("get_bank_dollars"))
		has_bank_balance = true
	elif _player_economy.has_method("get_dollars"):
		bank_balance = int(_player_economy.call("get_dollars"))
		has_bank_balance = true

	if _player_economy.has_method("get_cash_dollars"):
		cash_balance = int(_player_economy.call("get_cash_dollars"))
		has_cash_balance = true
	elif _player_economy.has_method("get_dollars"):
		cash_balance = int(_player_economy.call("get_dollars"))
		has_cash_balance = true

	balance_label.text = "На счете: %s" % (_format_money(bank_balance) if has_bank_balance else "--")
	cash_label.text = "Наличные: %s" % (_format_money(cash_balance) if has_cash_balance else "--")
...
func _update_notification() -> void:
	...
	if _player_economy == null:
		notification_lines.append("PlayerEconomy недоступен. Балансы не загружены.")
...
func _on_bank_dollars_changed(new_value: int) -> void:
	balance_label.text = "На счете: %s" % _format_money(new_value)
